...
const Departement = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/ufr/${id}`);
      setDeps(response.data.departements);
      setLoading(false);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.id) {
        ...
        await axios.post(`http://localhost:3000/ufr/${id}/departement`, newDepart);
      }
      setFormData({
        id:'',
        libelle:'',
  ...
  const handleEdit = (dep) => {
    setFormData({
      id: dep.id,
      libelle:dep.libelle,
      abreviation:dep.abreviation,
      email:dep.email,
      type:dep.type,
      dateCreation:dep.dateCreation,
      arreteCreation:dep.arreteCreation
    });
    setIsAdding(true);
  };
  ...
  const handleDelete = async (id) => {
    try {
        if (window.confirm("Êtes-vous sûr de vouloir supprimer cette departement ?")) {
            await axios.delete(`http://localhost:3000/ufr/${id}/${formData.id}`);

            const nouveauxDepart= deps.filter(dep => dep.id !== id);
            setDeps(nouveauxDepart);
        }
    } catch (error) {
  ...
  const columns = [
    ...
    { field: 'dateCreation', headerName: 'Date de Création', width: 200 },
    { field: 'arreteCreation', headerName: 'Arrêté de Création', width: 200 },
    {
      field: 'edit',
      headerName: 'Modifier',
      width: 150,
  ...
  const filteredDep = Array.isArray(deps) ? deps.filter(dep => {
    return dep.libelle.toLowerCase().includes(searchQuery.toLowerCase()) ||
      dep.abreviation.toLowerCase().includes(searchQuery.toLowerCase());
  }) : [];
